## SS GHS ATOMIC LOCAL -- SECOND ROUND WITH MEDMIX CVS GIVEN (NO FPC)
...
############## LOAD IN PARAMETERS FROM MEDMIX_GA OUTPUT
...
cat(sprintf("\n>> optimStrata (atomic) finished in %.2f minutes.\n", as.numeric(time_taken)))
...
filename <- paste0(
  "OUTPUT/ss_ghs_atomic2_seed", seed, "_results_LOCALTEST.Rdata"
)
