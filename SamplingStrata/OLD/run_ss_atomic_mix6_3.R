...
set.seed(90725) ## Setting seed for var.bin function
d_mix6$V1_cat <- var.bin(d_mix6$V1,15, iter.max=15)
d_mix6$V2_cat <- var.bin(d_mix6$V2,15, iter.max=15)

datasets <- c("d_mix6")
dfs <- list(d_mix6)
type <- list(type6)
type_name <- list(type_name6)
num_strata = 5
ssize = 500
nvars = length(type_name6)
...
df_cv <- df_cv_min %>%
  ...
  select(cv_y_1, cv_y_2, cv_b_1, cv_b_2)
...
results <- foreach(iter = 1:40, .packages = c("dplyr", "haven", "SamplingStrata")) %dopar% {
  ...
  fitness <- calculate_variance(df=df_sol, df_sol$strata, vars = vars2, n=n, type=data_type, type_name=data_name, seed = iter)
...
filename = paste0("OUTPUT/ss_atomic", datasets, "_", num_strata, "strata", "_", ssize, "n", "_results.Rdata")
